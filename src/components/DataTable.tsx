            ...
            <tbody className="divide-y divide-gray-100">
              ...
              {viewMode === 'monthly' && monthlyRows.map((m, idx) => (
                <tr key={`m-${idx}`} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 font-medium">{formatCurrency(m.totalIncome - m.insurance)}</td>
                  <td className="px-4 py-3">{formatCurrency(m.spending)}</td>
                  <td className="px-4 py-3 font-medium text-red-600">{formatCurrency(m.gap)}</td>
