          ...
          <table className="w-full text-sm text-left relative">
            <thead className="bg-gray-50 text-gray-600 font-medium border-b border-gray-200 sticky top-0 z-10">
              <tr>
                ...
                <th className={`px-3 py-3 ${COL_TAXES}`} title="Tax on pension, salary & Social Security income">Income<br/>Tax<br/><span className="text-[10px] font-normal">(deducted)</span></th>
                ...
                <th className={`px-3 py-3 font-bold text-gray-700`} title="Essential + Discretionary only. Healthcare and taxes already removed from Net Income above.">Total Spend ⓘ<br/><span className="text-[10px] font-normal text-gray-400">Ess + Disc only</span></th>
              ...
              {viewMode === 'yearly' && yearlyRows.map((y) => {
                ...
                return (
                  <tr key={`y-${y.age}`} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-3 py-2.5 font-bold" title={`Essential ${formatCurrency(y.totalEssentialSpending)} + Discretionary ${formatCurrency(y.totalDiscretionarySpending)}`}>{formatCurrency(y.totalEssentialSpending + y.totalDiscretionarySpending)}</td>
              ...
              {viewMode === 'monthly' && monthlyRows.map((m, idx) => {
                ...
                return (
                  <tr key={`m-${idx}`} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-3 py-2 font-bold" title={`Essential ${formatCurrency(m.essentialSpending)} + Discretionary ${formatCurrency(m.discretionarySpending)}`}>{formatCurrency(m.essentialSpending + m.discretionarySpending)}</td>
